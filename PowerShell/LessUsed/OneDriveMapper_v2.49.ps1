...
function versionCheck{
    ...
    $apiURL = "http://www.lieben.nu/liebensraum/liebenapi/"
    $apiKeyword = "latestOnedriveMapperVersion"
    try{
        $result = Invoke-WebRequest -Uri $apiURL -TimeoutSec 10 -ErrorAction Stop -WarningAction Stop
    }catch{
...
function checkErrorAtLoginValue{
    ...
    if($mode -eq "msonline"){
        ...
        try{
            $found_ErrorControl = $script:ie.Document.getElementById("errorText").innerHTML
        }catch{$Null}
    }
...
function login(){ 
    ...
    if($useADFS -eq $False){ 
        ...
        $pwdAttempts = 0
        $trySecondgetElementMethod = $False
        while($pwdAttempts -lt 3){
            $pwdAttempts++
            try{ 
                if($script:ie.document.getElementById("cred_keep_me_signed_in_checkbox").checked -eq $False){
                    $script:ie.document.getElementById("cred_keep_me_signed_in_checkbox").click() 
                    log -text "Signin Option persistence selected using normal element"
                }else{
                ...
                $trySecondgetElementMethod = $True
                if((checkIfAtO365URL -url $script:ie.LocationURL -finalURLs $finalURLs)){
                    #we've been logged in, we can abort the login function 
                    log -text "login detected, login function succeeded, final url: $($script:ie.LocationURL)" 
                    return $True 
            ...
            if($trySecondgetElementMethod){
                try{ 
                    if($script:ie.document.IHTMLDocument3_getElementById("cred_keep_me_signed_in_checkbox").checked -eq $False){
                        $script:ie.document.IHTMLDocument3_getElementById("cred_keep_me_signed_in_checkbox").click() 
                        log -text "Signin Option persistence selected"
                    }else{
                        log -text "Signin Option persistence was already checked"
                    }
                    if($script:ie.document.IHTMLDocument3_getElementById("cred_keep_me_signed_in_checkbox").checked -eq $False){
                        log -text "the cred_keep_me_signed_in_checkbox is not selected! This may result in error 224" -fout
                    }    
                    if($pwdAttempts -gt 1){
                        if($userLookupMode -eq 4){
                            $userName = (retrieveLogin -forceNewUsername)
                            $script:ie.document.IHTMLDocument3_getElementById("cred_userid_inputtext").value = $userName
                        }
                        $script:ie.document.IHTMLDocument3_getElementById("cred_password_inputtext").value = retrievePassword -forceNewPassword
                    }else{
                        $script:ie.document.IHTMLDocument3_getElementById("cred_password_inputtext").value = retrievePassword 
                    }
                    $script:ie.document.IHTMLDocument3_getElementById("cred_sign_in_button").click() 
                    waitForIE
                }catch{ 
                    sleep -s 2
                    if((checkIfAtO365URL -url $script:ie.LocationURL -finalURLs $finalURLs)){
                    ...
                    log -text "Failed to find the correct controls at $($ie.LocationURL) to log in by script, check your browser and proxy settings or check for an update of this script (3). $($Error[0])" -fout
                    abort_OM
                }                
            }
            Sleep -s 1
            ...
            while($pwdAttempts -lt 3){
                ...
                try{ 
                    ...
                    if($adfsMode -eq 1){
                        ...
                        $script:ie.document.getElementById($adfsLoginInput).value = ($userName.Split("@")[0])
                    }
                    if($pwdAttempts -gt 1){
                        $script:ie.document.getElementById($adfsPwdInput).value = retrievePassword -forceNewPassword
                    }else{
                        $script:ie.document.getElementById($adfsPwdInput).value = retrievePassword 
                    }
...
if($showProgressBar) {
    ...
    $label1.text="Please wait while we connect your drives..."
    $label1.Name = "label1"
    $label1.Left=5
    $label1.Top= 5
...
if($versionCheck){
    #update progress bar
    if($showProgressBar) {
        $form1.controls["Label2"].Text = "Performing version check..."
        $form1.Refresh()
    }
    try{
        ...
        if($showProgressBar) {
            $form1.controls["Label2"].Text = "You are running the latest version :)"
            $form1.Refresh()
        }
    }catch{
        if($showProgressBar) {
            $form1.controls["Label2"].Text = "This is not the latest version :("
            $form1.Refresh()
            Sleep -s 3
        }
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "Running some checks..."
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "Determining your mappings..."
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "Logging in to Office 365..."
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "Logged in! Detecting target mapping..."
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "Eating some cookies....:)"
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "Almost done!"
    $script:progressbar1.Value = 80
    $script:form1.Refresh()
}
...
#update progress bar
if($showProgressBar) {
    $form1.controls["Label2"].Text = "And where there! Bye now :)"
